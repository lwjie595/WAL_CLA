...
def Each_labelloss(F1, feat_source, feat_untarget,source_label,lamda_w,lambda_cla,thred, temp,eta=1.0):
    ...
    for i in range(Label_num):
        ...
        if (source_index.shape[0]>0) and (target_index.shape[0]>0):
            ...
            loss_each_label+=torch.exp(F.cosine_similarity(mean_source,mean_target,dim=0)/temp)
    loss_each_label=torch.log(loss_each_label+torch.tensor(1e-6))*lambda_cla
    loss_ent = lamda_w * torch.mean(torch.mean(F.sigmoid(out_t1),0)-torch.mean(F.sigmoid(out_t2),0))

    return loss_ent-loss_each_label
